...
namespace AWSGsrSerDe.Tests.deserializer.avro
{
    ...
    public class AvroDeserializerTest
    {
        ...
        private const string AvroEnumSchema = "resources/avro/user_enum.avsc";
        private const string AvroArraySchema = "resources/avro/user_array.avsc";
        private const string AvroStringArraySchema = "resources/avro/user_array_String.avsc";
        private const string AvroUnionSchema = "resources/avro/user_union.avsc";
        private const string AvroFixedSchema = "resources/avro/user_fixed.avsc";
        private const string AvroMapSchema = "resources/avro/user_map.avsc";
        private const string AvroMxiedTypeSchema = "resources/avro/user3.avsc";
        ...
        public void TestDeserialize_SpecificRecord_Succeed()
        {
            ...
            var deserialize = (User) avroDeserializer.Deserialize(serializedResult,
        ...
        public void TestDeserialize_SpecificRecordIntoGenericRecord_Succeed()
        {
            ...
            var deserialize = (GenericRecord) avroDeserializer.Deserialize(serializedResult,
                SpecificRecordSchema);

            Assert.AreEqual(_userDefinedSpecificRecord.Get(0), 
                deserialize.GetValue(deserialize.Schema["name"].Pos));
            Assert.AreEqual(_userDefinedSpecificRecord.Get(1), 
                deserialize.GetValue(deserialize.Schema["favorite_number"].Pos));
            Assert.AreEqual(_userDefinedSpecificRecord.Get(2), 
                deserialize.GetValue(deserialize.Schema["favorite_color"].Pos));
        }
        
        [Test]
        public void TestDeserialize_EnumSchema_Succeed()
        {
            var enumSchema = SchemaLoader.LoadAvroSchema(AvroEnumSchema);
        ...
        public void TestDeserialize_IntegerArrays_Succeed()
        {
            ...
            var item = 1;
            var arraySchema = SchemaLoader.LoadAvroSchema(AvroArraySchema);
        ...
        public void TestDeserialize_StringArrays_Succeed()
        {
            // We do not have a easy way to serialize arrays due to a limitation in Avro library
            var item = "TestValue";
            var arraySchema = SchemaLoader.LoadAvroSchema(AvroStringArraySchema);
            var intArray = new string[]{item};
            var defaultWriter = new DefaultWriter(arraySchema);
            var memoryStream = new MemoryStream();
            Encoder encoder = new BinaryEncoder(memoryStream);
            defaultWriter.Write(intArray,encoder);
        ...
        {
            var unionSchema = SchemaLoader.LoadAvroSchema(AvroUnionSchema);
        ...
        {
            var fixedSchema = SchemaLoader.LoadAvroSchema(AvroFixedSchema);
        ...
        public void TestDeserialize_Maps_Succeed()
        {
            ...
            var mapSchema = SchemaLoader.LoadAvroSchema(AvroMapSchema);
        ...
        public void TestDeserialize_AllTypes_Succeed()
        {
            ...
            var mixedSchema = (RecordSchema) SchemaLoader.LoadAvroSchema(AvroMxiedTypeSchema);
        ...
        public void TestDeserialize_UnknownRecordType_ThrowsException()
        {
            ...
            var exception =Assert.Throws(typeof(AwsSchemaRegistryException), 
                ()=>avroDeserializer.Deserialize(serializedResult,GenericRecordSchema));
        ...
        public void TestDeserialize_InvalidSchema_ThrowsException()
        {
            ...
            var invalid = new GlueSchemaRegistrySchema("invalid","invalid", GlueSchemaRegistryConstants.DataFormat.AVRO.ToString());

            Assert.Throws(typeof(AwsSchemaRegistryException), 
